...
#DQN
parser.add_argument("--explore_rate",default= 0.5,type=float)
...
class PPOAgent:
    ...
    def sample_many(self, mbsize, all_visited):
        ...
        while not all(done):
            ...
            """
            Q-learning 与 policy gradient method是不同的。Policy Gradient的output是随机的，从distribution忠sample出一个action
            而Q-learning 是给定一个state，穷举所有的action，采用Q-value最大的action （ epsilon-greedy ) 
            """
            step = [i.step(a) for i,a in zip([e for d, e in zip(done, self.envs) if not d], acts)]
        ...
        for tau in trajs.values():
            ...
            adv = r + vsp * (1-d) - vs  #  adv = r + vsp * (1-d)* gamma - vs , here gamma is 1
    ...
    def learn_from(self, it, batch):
        ...
        o = self.model(s)
        logits, values = o[:, :-1], o[:, -1] # values : obj_entropy
        ...
        new_logprob = new_pol.log_prob(a)
         ...
         #  new_logprob = -(self.a_std_log + self.sqrt_2pi_log + delta).sum(1)
         #  obj_entropy  = (logprob.exp() * logprob).mean()

        """
        ElegantRL https://github.com/AI4Finance-Foundation/ElegantRL/blob/master/elegantrl/agents/AgentPPO.py , 211
        """
        ratio = torch.exp(new_logprob - lp)  # lp : logprob
        surr1 = ratio * A
        surr2 = torch.clamp(ratio, 1.0 - self.clip_param,
                            1.0 + self.clip_param) * A
        action_loss = -torch.min(surr1, surr2).mean()  # obj_surrogate = -torch.min(surrogate1, surrogate2).mean()

        #  ---------
        """
        ElegantRL https://github.com/AI4Finance-Foundation/ElegantRL/blob/master/elegantrl/agents/net.py , 352
        """
        value_loss = 0.5 * (G - values).pow(2).mean()
        """
        github.com/datawhalechina/easy-rl/blob/master/codes/PPO/agent.py 79
        
        actor_loss =  -torch.min(weighted_probs, weightes_clipped_probs).mean()
        critic_loss = (returns - critic_value) **2
        critic_loss = critic_loss.mean()
        total_loss = actor_loss + 0.5 * critic_loss
        """
        entropy = new_pol.entropy().mean()  # obj_entropy  = (logprob.exp() * logprob).mean()
...
class DQNAgent:
    ...
    def sample_many(self,mbsize,all_visited):
        ...
        while not all(done):
            with torch.no_grad():
                q_values = self.model_policy_net(s)

                if rd.random() > self.explore_rate :
                    acts = torch.randint(high = args.ndim,size=(s.shape[0],))
                    #acts = torch.tensor([[random.randrange(args.ndim)]],dtype = torch.long)

                else:
                    #acts = q_values.argmax(dim=1)
                    acts = q_values.max(1)[1].view(s.shape[0],)
    ...
    def learn_from(self,it,batch):
        ...
        # o_next = self.model_target_net(sp)

        q_values_next = self.model_target_net(sp).max(1)[0].detach()



        expected_q_values = r + self.reward_decay * q_values_next*(1-d)
